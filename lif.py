...
def run_optimized():
    """Entry point for the optimized Lif simulation"""
    # Parse extended arguments
    extended_args, original_args = parse_extended_args()
    
    # Apply optimizations by default unless explicitly disabled
    if not hasattr(extended_args, 'no_optimize') or not extended_args.no_optimize:
        optimize_functions()
